...
def single_crash_email(df):
    ...
    if location == "":
        location = str(df.loc[0, 'on_street_name']) + " & " + str(df.loc[0, 'off_street_name'])
    ...
    msg = MIMEText(f"""Dear Council Member,
    ...
    msg["To"] = "farhanazam98@outlook.com"
...
def send_injury_email(df, council_district):
    ...
    df_injury_cd_current = df[(df['crash_date'] == today) & 
